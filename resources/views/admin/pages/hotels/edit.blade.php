...
@section('content')
                ...
                <div class="card-body">
                    <form action="{{ route('hotels.update', $hotel->id) }}" method="POST" id="hotelForm">
                            ...
                            <div class="col-md-6 mb-3">
                                <div class="form-check mt-4">
                                    <input class="form-check-input" type="checkbox" value="1" id="is_active"
                                        name="is_active" {{ old('is_active', $hotel->is_active) ? 'checked' : '' }} />
                        ...
                        <div id="bankAccountsContainer">
                            @forelse($hotel->bankAccounts as $index => $account)
                                <div class="bank-account-row mb-3 p-3 border rounded">
                                    <div class="row">
                                        <div class="col-md-4">
                                            <label class="form-label">{{ __('Currency') }}</label>
                                            <select name="bank_accounts[{{ $index }}][currency_id]"
                                                class="form-select">
                                                <option value="">{{ __('Select Currency') }}</option>
                                                @foreach ($currencies as $currency)
                                                    <option value="{{ $currency->id }}"
                                                        {{ $account->currency_id == $currency->id ? 'selected' : '' }}>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <input type="text" name="bank_accounts[{{ $index }}][account_number]"
                                                class="form-control" value="{{ $account->account_number }}" />
                                        </div>
    ...
    <script>
        let accountIndex = {{ $hotel->bankAccounts->count() > 0 ? $hotel->bankAccounts->count() : 1 }};
